    ...
    def update(self, settings, servoModeIn):
        ...
        if servoModeIn and self.stage < 3:
            ...
            if self.heatSwitch.ready:
                logging.info(
                    "Closing heat switch in preparation for autocycle.")
            ...
            if self.heatSwitch.ready and self.heatSwitch.hswError is None and self.heatSwitch.hswClosed:
                logging.info(
                    "Hsw successfully closed. Autocycle ntering stage 1 - wait for hsw toggle."
                )
                self.stage = 1
            ...
            if now > settings['start_time'] + timedelta(
                ...
                self.heatSwitch.check_status()
                if not settings['do_hsw_toggle'] or (
                        self.heatSwitch.ready and self.heatSwitch.hswError is None):
                    self.stage = 2
                    logging.info(
            ...
            ramprate = settings['ramprate_up']
            if now > settings['start_time'] + timedelta(
                    hours=settings['duration']):
                if self.heatSwitch.ready:
                    logging.info("Opening hsw in preparation for demag.")
                self.heatSwitch.openHsw()

            self.heatSwitch.check_status()
            if not self.heatSwitch.hswClosed and self.heatSwitch.ready and self.heatSwitch.hswError is None:
                logging.info(
                    "Hsw opened successfully. Autocycle entering stage 3 - drain current."
                )
                self.stage = 3
        ...
        return (current, ramprate, servoMode, {
            'done': self.done,
            'stage': self.stage,
